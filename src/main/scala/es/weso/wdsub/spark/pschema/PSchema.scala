...
/** Pregel Schema validation
  ...
  *   type of errors
  * @tparam E
  *   type of property identifiers
  *
  * @param checkLocal
  *   returns checks if the shape expression associated with label can validate
  *   a node locally. It returns Left(err) if the node doesn't validate
  *   Right(labels) if the node's validation depends on a set of labels. If that
  *   set is empty it just validates checkNeighs(label, bag, failed)
  * @param checkNeighs
  *   checks if the regular bag expression associated with `label` matches
  *   `bag`. Failed contains a set of pairs (node, label) which have already
  *   failed getTripleConstraints(label)
  * @param getTripleConstraints
  *   returns the triple constraints associated with label in the schema cnvEdge
  *   converts a node in the graph to a node
  * @param cnvEdge
  *   a function that converts the type of edges to the type of arcs employed
  *   in the schema
  * @param verbose
  */
class PSchema[VD: ClassTag, ED: ClassTag, L: Ordering, E, P: Ordering](
  ...
  def vprog(
    ...
    val newValue = msg.mmap.foldLeft(v) { case (acc, (lbl, msgLabel)) =>
      vProgLabel(acc, lbl, msgLabel)
    }
    info(
      s"""|vProg(${id})
             |Msg:    $msg
  ...
  private def sendMsg(
      t: EdgeTriplet[Shaped[VD, L, E, P], ED]
  ): Iterator[(VertexId, MsgMap[L, E, P])] = {
    // active shapes = waitingFor/pending
  ...
  private def sendMessagesActive(
    ...
    filteredTcs.toIterator.map { case (p, l) =>
  ...
  private def sendMessagesTripleConstraint(
    ...
    val msgs =
      if (obj.okShapes contains label) {
        ...
        val es = NonEmptyList.fromListUnsafe(
          obj.failedShapes.map(_._2.toList).flatten.toList
        )
        List(
          (
            triplet.srcId,
            notValidatedMsg(pendingLabel, p, label, triplet.dstId, es)
          )
        )
      } else {
        //
    ...
    info(
      s"""|Msgs for triplet (src =${triplet.srcId}, attr=${triplet.attr}, dst=${triplet.dstId}):
             |${msgs.mkString("\n")}
             |""".stripMargin,
      verbose
    )
  ...
  private def mergeMsg(
      p1: MsgMap[L, E, P],
      p2: MsgMap[L, E, P]
  ): MsgMap[L, E, P] = p1.merge(p2)

  private def shapedGraph(
      graph: Graph[VD, ED]
  ...
  def vProgLabel(
    ...
    msgLabel match {
      ...
      case ValidateLabel =>
        acc.statusMap.get(lbl) match {
          case None | Some(Pending) =>
            checkLocal(lbl, acc.value) match {
      ...
      case wf: WaitFor[L, P] =>
        acc.statusMap.get(lbl) match {
          case None | Some(Pending) =>
            acc.withWaitingFor(lbl, wf.ds, Set(), Set())
  ...
  private def mkBag[P: Ordering, L: Ordering](
      s: Set[DependTriple[P, L]]
  ): Bag[(P, L)] =
    Bag.toBag(s.map { case t => (t.prop, t.label) })

  private def mkFailed[P, E, L](
      s: Set[(DependTriple[P, L], NonEmptyList[E])]
  ): Set[(P, L)] =
    s.map { case (t, _) => (t.prop, t.label) }

  private def checkRemaining(
      id: VertexId,
      v: Shaped[VD, L, E, P]
  ): Shaped[VD, L, E, P] = {
    val v1 = v.pendingShapes.foldLeft(v) {
      case (acc, pendingLabel) => {
        ...
        info(
          s"""|------------checkRemaining($id) pendingShapes with pendingLabel $pendingLabel
                     |checkNeighs(
                     | lbl= $pendingLabel, 
                     | bag = {||}, 
                     | failed = {}) = 
    ...
    val result = v1.waitingShapes.foldLeft(v1) {
      case (acc, (lbl, wf: WaitingFor[_, L, E, P])) => {
        // We are assuming that the rest of dependents are valid, not sure if that's ok
        val bag = mkBag(wf.validated ++ wf.dependants)
        val failed = mkFailed(wf.notValidated)
        val resultCheckNeighs = checkNeighs(lbl, bag, failed)
        info(
          s"""|------------checkRemaining($id) waitingShapes with label $lbl
                 |checkNeighs(
                 | lbl= $lbl, 
                 | bag = {||}, 
                 | failed = {}) = 
  ...
  def runPregel(
      ...
      verbose: Boolean = false // TODO: make verbose work :D
  ): DataFrame = {
    graph.pregel
      .withVertexColumn(
        initialLabel.toString(), // TODO: is this fine?
        initialMsg(initialLabel.toString()),
        vprog
      )
      .sendMsgToDst(sendMsgToDst)
      .aggMsgs(aggMsgs)
      .setMaxIter(maxIterations)
      .run()
  }

  private def initialMsg(initialLabel: String): Column = {
    // TODO: Implement this :(
    throw new NotImplementedException
  }

  private def vprog(): Column = {
    // TODO: Implement this :(
    throw new NotImplementedException
  }

  private def sendMsgToDst(): Column = {
    // TODO: Implement this :(
    throw new NotImplementedException
  }

  private def aggMsgs(): Column = {
    // TODO: Implement this :(
    throw new NotImplementedException
  }
...
object PSchema extends Serializable {
  ...
  /** Execute a Pregel-like iterative vertex-parallel abstraction following a
    * validating schema.
    *
    * On the first iteration all vertices receive the `initialMsg` and on
    * subsequent iterations if a vertex does not receive a message then the
    * vertex-program is not invoked.
    *
    * This function iterates until there are no remaining messages, or for
    ...
    *   the edge data type
    * @tparam L
    *   the type of labels in the schema
    * @tparam E
    *   the type of errors that happen when validating
    * @tparam P
    *   the type of properties (arcs in the graph)
    *
    * @param graph
    *   the input graph.
    * @param initialLabel
    ...
    * @param checkLocal
    *   the function that validates locally a vertex against a label in the
    *   schema it returns either an error or if it validates, a set of pending
    *   labels If there is no pending labels, the set will be empty
    * @param checkNeighs
    *   it checks the bag of neighbors of a node against the regular bag
    *   expression defined by the label in the schema The third parameter
    *   contains the triples that didn't validate
    * @param getTripleConstraints
    *   returns the list of triple constraints associated with a label in a
    *   schema. A triple constraint is a pair with an arc and a pending label
    * @param cnvProperty
    *   a function that converts the type of edges to the type of arcs employed
    *   in the schema
    *
    * @return
    *   the resulting graph at the end of the computation with the values
    *   embedded in a `ShapedValue` class that contains information about ok
    *   shapes, failed shapes, inconsistent shapes and pending shapes.
    */
