    ...
    vertexEdges(
      List(
        ...
        tripleq(
          ...
          cern,
          List(
            LocalQualifier(start.prec.id, y1980),
            LocalQualifier(end.prec.id, y1980)
          )
        ),
        tripleq(
          timBl,
          ...
          cern,
          List(
            LocalQualifier(start.prec.id, y1984),
            LocalQualifier(end.prec.id, y1994)
          )
        ),
        tripleq(
          timBl,
          ...
          paAward,
          List(
            EntityQualifier(togetherWith.prec.id, vintCerf),
            LocalQualifier(time.prec.id, y2002)
          )
        ),
        ...
        tripleq(
          ...
          awardReceived,
          paAward,
          List(LocalQualifier(time.prec.id, y2013))
        )
      )
    )
  }
    ...
    Map(
          ...
          Some(
            EachOf(
              ...
              List(
                TripleConstraintRef(
                  Pid(31),
                  WShapeRef(None, IRILabel(IRI("Human"))),
                  1,
                  IntLimit(1)
                ),
                TripleConstraintLocal(
                  Pid(569),
                  WNodeConstraint.datatype(`xsd:dateTime`),
                  0,
                  IntLimit(1)
                ),
                TripleConstraintRef(
                  Pid(19),
                  WShapeRef(None, IRILabel(IRI("Place"))),
                  1,
                  IntLimit(1)
                )
              )
        ...
        Some(
          EachOf(
            ...
            List(
              TripleConstraintRef(
                Pid(17),
                WShapeRef(None, IRILabel(IRI("Country"))),
                1,
                IntLimit(1)
              )
            )
      ...
      IRILabel(IRI("Human")) ->
        WNodeConstraint.valueSet(
          List(
            EntityIdValueSetValue(
              ItemId("Q5", IRI("http://www.wikidata.org/entity/Q5"))
            )
          )
        )
    )
