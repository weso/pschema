...
object DumpUtils {
  ...
  lazy val PropertyIdDisplacement: Long =
    10000000000L // We will assign vertex id's for items starting from 0 and for properties starting with this value
  lazy val StringIdDisplacement: Long =
    20000000000L // We will assign vertex id's for items starting from 0 and for properties starting with this value
  ...
  def mkEntity(ed: EntityDocument): Vertex[Entity] = {
    ...
    ed match {
      case id: ItemDocument =>
        Vertex(
          vertexId,
          Item(
            ItemId(id.getEntityId().getId(), IRI(id.getEntityId().getIri())),
            VertexId(vertexId),
            Map(Lang("en") -> Option(id.findLabel("en")).getOrElse("")),
            Map(),
            Map(),
        ...
        Vertex(
          ...
          Property(
            PropertyId(
              ...
              IRI(pd.getEntityId().getIri())
            ),
            VertexId(vertexId),
            Map(Lang("en") -> Option(pd.findLabel("en")).getOrElse("")),
            Map(),
            Map(),
      ...
      case ev: EntityIdValue => {
        ...
        Some(
          Edge(
            subjectId,
            valueId,
            Statement(PropertyRecord(pid, VertexId(pVertex)))
          )
        )
      }
      case _ => None
